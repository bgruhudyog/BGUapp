...
export default function TotalRemainingPage() {
  ...
  useEffect(() => {
    fetchRoutes();
    fetchAllShopsData();
    // Load message timestamps from localStorage
  ...
  const fetchAllShopsData = async () => {
    ...
    if (error) console.error("Error fetching all shops data:", error);
      ...
      const globalTotal = data.reduce((acc, shop) => {
        return (
          acc +
          calculateTotalRemaining(shop.total, shop.total_cash, shop.total_old)
        );
      }, 0);
  ...
  const isMessageDisabled = (shopId) => {
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell align="right">संदेश भेजें</TableCell>
    ...
    <Box sx={{ padding: 2 }}>
      ...
      {selectedRouteName && (
        <Typography variant="subtitle1" align="center" sx={{ mb: 2 }}>
          {selectedRouteName}: कुल उधारी ₹{totalRemaining.toFixed(2)}
        </Typography>
      )}
      {shopData.length > 0 &&
